...
public class ChunkBlanketings {
    ...
    public static final Codec<ChunkBlanketingProcessor> DISPATCH_CODEC = TYPE_CODEC.dispatch("chunk_surface_modifier_type", ChunkBlanketingProcessor::getType, ChunkBlanketingType::getCodec);

    public static final DeferredHolder<ChunkBlanketingType, ChunkBlanketingType> PROTOTYPE_MODIFIER = CHUNK_BLANKETING_TYPES.register("prototype", () -> () -> PrototypeBlanketingProcessor.CODEC);
    ...
    public static void bootstrap(BootstapContext<ChunkBlanketingProcessor> context) {
        ...
        context.register(TEST, new PrototypeBlanketingProcessor(lookup.getOrThrow(BiomeTags.STRONGHOLD_BIASED_TO), BlockStateProvider.simple(Blocks.RED_STAINED_GLASS), 32));
        context.register(RIVER_TEST, new PrototypeBlanketingProcessor(lookup.getOrThrow(BiomeTags.IS_RIVER), BlockStateProvider.simple(Blocks.GLASS), 16));
    }
}
